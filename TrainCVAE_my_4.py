...
class CVAE(object):
    ...
    def __init__(self, sketchy_data, max_epoch=25, batch_size=1024, summary_path="summary"):
        ...
        self.decoder, self.decoder_2, self.vae = self.build()
    ...
    def build(self):
        ...
        vae = Model(inputs=[sketch_features, image_features, self.image_diff_features],
                    outputs=[d_out_recons, recons_sketch])
        ...
        vae.compile(optimizer=Adam(lr=0.0002, beta_1=0.5, beta_2=0.999, epsilon=1e-08, decay=0.0),
                    ...
                    loss_weights={'decoder_out': 10.0, 'recons_output': 1.0})

        return decoder, decoder_2, vae
    ...
    def _d_diff_loss(self, y_true, y_pred):
        ...
        # P2
        same_recon_2 = k.mean(k.square(self.d_2_out_recons - self.d_2_out_same_recons), axis=1)
        diff_recon_2 = k.mean(k.square(self.d_2_out_recons - self.d_2_out_diff_recons), axis=1)
        d_2_loss = k.maximum(2. + same_recon_2 - diff_recon_2, 0.)

        # P1 + P2
        return d_1_loss + d_2_loss
    ...
    def test(self, epoch):
        ...
        def cal_result(_predict_image_features, _img_vgg_features_ext):
            ...
            return precision_neigh_num, m_ap
    ...
    def test2(self):
        ...
        def cal_result(_search_image_features, _search_image_class, _predict_image_features, _test_image_class, info):
            ...
            # 输出
            print('{}'.format(info))
            print('The mean precision@200 for test sketches is {}'.format(_precision_neigh_num))
            print('The mAP for test_sketches is {}'.format(_m_ap))
    ...
    def run2(self, model_file, load_weights=True):
        # 载入模型
        if os.path.exists(model_file) and load_weights:
            self.vae.load_weights(model_file, skip_mismatch=True)
        # 测试
...
if __name__ == '__main__':
    ...
    CVAE(sketchy_data=SketchyData(n_x=4096, n_y=4096 * 2), max_epoch=100,
         batch_size=1024, summary_path="model/{}/summary".format(name)).run(
        model_file="model/{}/model.h5".format(name), load_weights=False)
