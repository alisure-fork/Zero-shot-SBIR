...
class CVAE(object):
    ...
    def build(self):
        ...
        def net_decoder(_sketch_features, _input_z):
            d_in = layers.concatenate([_input_z, _sketch_features])
        ...
        # 解码层
        decoder_hidden = layers.Dense(self.hidden_size, activation='relu')
        decoder_out = layers.Dense(self.n_x, activation='relu', name='decoder_out')
        ...
        _recons_int = layers.Dense(self.hidden_size, activation='relu')(recons_image)
    ...
    def _recons_diff_loss(self, y_true, y_pred):
        same_recon = k.mean(k.square(y_pred - y_true), axis=1)
        diff_recon = k.mean(k.square(y_pred - self.image_diff_features), axis=1)
        return k.maximum(2. + same_recon - diff_recon, 0.)

    @staticmethod
    def _recons_sketch_loss(y_true, y_pred):
...
if __name__ == '__main__':
    ...
    CVAE(sketchy_data=SketchyData(n_x=4096, n_y=4096 * 2), max_epoch=100,
        ...
        model_file="model/{}/model.h5".format(name), load_weights=True)
