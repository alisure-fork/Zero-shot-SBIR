...
vae.compile(optimizer=adam, loss={'decoder_out': vae_loss, 'recons_output': 'mean_squared_error'},
            loss_weights={'decoder_out': 1.0, 'recons_output': 10.0})

# ============================Load the required data================================== #

data_path = "./data/ZSSBIR_data"
...
sketch_paths = [_.decode() for _ in np.load(os.path.join(data_path, 'sketch_paths.npy'))]  # (,)
sketch_VGG_features = np.load(os.path.join(data_path, 'sketch_vgg_features.npy'))  # (,)

# next load the image extension dataset
image_VGG_features_ext = np.load(os.path.join(data_path, 'image_ext_vgg_features.npy'))  # (73002, 4096)
image_paths_ext = [_.decode() for _ in np.load(os.path.join(data_path, 'image_ext_paths.npy'))]  # (73002, )
...
test_ref_classes = [_.decode() for _ in np.load(os.path.join(data_path, 'test_split_ref.npy'))]
...
def find_precision():
    ...
    for ii in range(0, len(test_sketch_paths)):
        avgPredImgFeatures[ii] = np.mean(predImageFeatures[ii * RANDOM_Z_PER_SKETCH:(ii + 1) * RANDOM_Z_PER_SKETCH],
                                         axis=0)
...
def find_precision_min():
    ...
    for ii in range(PRED_BATCH_SIZE):
        distances[ii * PRED_BATCH_SIZE:(ii + 1) * PRED_BATCH_SIZE], indices[ii * PRED_BATCH_SIZE:(
                                                                                                 ii + 1) * PRED_BATCH_SIZE] = nbrs.kneighbors(
            predImageFeatures[ii * PRED_BATCH_SIZE:(ii + 1) * PRED_BATCH_SIZE])
