...
class SketchyData(object):
    ...
    def get_data(self):
        ...
        return (test_image_ext_classes, train_classes, train_sketch_classes, test_sketch_classes,
                ...
                train_sketch_x, train_x_img, test_sketch_paths, test_sketch_x, test_x_img)
...
class CVAE(object):
    ...
    def __init__(self, sketchy_data, max_epoch=25, batch_size=1024, summary_path="summary"):
        ...
        (self.image_classes, self.train_classes, self.train_sketch_classes, self.test_sketch_classes,
         ...
         self.test_sketch_paths, self.test_sketch_x, self.test_x_img) = sketchy_data.get_data()
    ...
    def build(self):
        ...
        decoder_2_out = layers.Dense(self.n_x, activation='relu', name='decoder_2_out')
        ...
        vae.compile(optimizer=Adam(lr=0.0002, beta_1=0.5, beta_2=0.999, epsilon=1e-08, decay=0.0),
                    ...
                    loss_weights={'decoder_out': 10.0, 'recons_output': 1.0, 'decoder_2_out': 0.0})
        ...
        vae_p2.compile(optimizer=Adam(lr=0.0001, beta_1=0.5, beta_2=0.999, epsilon=1e-08, decay=0.0),
                       loss={'decoder_out': self._d_diff_loss,
                             'recons_output': self._vae_loss,
                             'decoder_2_out': self._d_2_diff_loss},
                       loss_weights={'decoder_out': 10.0, 'recons_output': 1.0, 'decoder_2_out': 10.0})
    ...
    def test(self, epoch):
        ...
        print("{} {} {}".format("P1空间计算", precision_neigh_num, m_ap))
        ...
        print("{} {} {}".format("P2空间计算", precision_neigh_num, m_ap))
        print()
        pass
    ...
    def run(self, model_1_file, model_2_file, load_weights=True, save_weights=True):
        ...
        model_path = os.path.split(model_1_file)[0]
        if not os.path.exists(model_path):
            os.makedirs(model_path)
        model_path = os.path.split(model_2_file)[0]
        if not os.path.exists(model_path):
            os.makedirs(model_path)
...
P1空间计算 0.4336249409169686 0.2907438980066568
P2空间计算 0.43249763667874586 0.2946533103734533

使用原始测试图像（从VGG中提取）的图像特征，在增强的数据集中检索
P1空间计算 0.7329506065857886 0.6560596963331535
P2空间计算 0.04488774224042855 0.008920907592917787

预测测试图片的图像特征，在测试数据集中检索
P1空间计算 0.48983968804159445 0.3723114129313227
P2空间计算 0.4995501811879628 0.3856276148711154

使用原始测试图像（从VGG中提取）的图像特征，在测试数据集中检索
P1空间计算 0.8496328974318577 0.8044398214878347
P2空间计算 0.04875610524657318 0.014762769298528464

预测训练图片的图像特征，在训练集中检索
P1空间计算 0.8126346218166182 0.7542178724424441
P2空间计算 0.8029006800770861 0.7483915919191622

使用原始训练图像（从VGG中提取）的图像特征，在训练集中检索
P1空间计算 0.8688856769713476 0.8345214729457112
P2空间计算 0.00884585981174447 0.00108556177335484

预测测试图片的图像特征，在训练和测试集中检索
P1空间计算 0.12858909721128092 0.055829297721335526
P2空间计算 0.16823932566566882 0.08167711136426554

使用原始测试图像（从VGG中提取）的图像特征，在训练和测试集中检索
P1空间计算 0.6806124940916969 0.6030668249599961
P2空间计算 0.02387033244052308 0.004681607999113183

使用原始训练图像（从VGG中提取）的图像特征，在训练和测试集中检索
P1空间计算 0.8506080080271394 0.8122628671237255
P2空间计算 0.004004889547199262 0.00043128974118592184

预测训练图片的图像特征，在训练和测试集中检索
P1空间计算 0.7985087677385445 0.736466989073453
P2空间计算 0.7811106598499689 0.72063043370448
"""
...
if __name__ == '__main__':
    ...
    name = "triplet_fine_tune_p2"

    CVAE(sketchy_data=SketchyData(n_x=4096, n_y=4096 * 2), max_epoch=30,
